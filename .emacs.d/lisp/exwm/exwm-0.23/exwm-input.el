...
(defun exwm-input--update-mode-line (id)
  ...
  (let (help-echo cmd mode)
    (cl-case exwm--input-mode
      (line-mode
       (setq mode "line"
             help-echo "mouse-1: Switch to char-mode - aca"
             cmd (lambda ()
                   (interactive)
                   (exwm-input-release-keyboard id))))
      (char-mode
       (setq mode "char"
             help-echo "mouse-1: Switch to line-mode - aca" ;ne radi
             cmd (lambda ()
                   (interactive)
...
(defun exwm-input--grab-keyboard (&optional id)
  ...
  (when id
    ...
    (let ((buffer (exwm--id->buffer id)))
      (when buffer
        (with-current-buffer buffer
	  ;; aca https://github.com/ch11ng/exwm/pull/738/commits/c5c2d5150f8c74f0756b21dec814ded243d3fb01
...
(defun exwm-input-grab-keyboard (&optional id)
  ...
  (when id
    (exwm--log "id=#x%x" id)
    (setq exwm--selected-input-mode 'line-mode)	;aca gledao
    (
    (exwm-input--grab-keyboard id)
