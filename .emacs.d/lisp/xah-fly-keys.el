...
;;aca dodao
(xah-fly--define-keys
 (define-prefix-command 'xah-fly-java-keymap)
 '(
   ("t" . flymake-goto-next-error)
   ("i" . lsp-ui-peek-find-definitions)
   ;; ("n" . lsp-ui-peek-find-implementation)
   ("n" . lsp-treemacs-implementations)
   ;; ("o" . lsp-ui-peek-find-references)
   ("o" . lsp-treemacs-references)
   ;; ("," . flymake-show-diagnostics-buffer) ;;nekad
   ("," . lsp-treemacs-errors-list)
   ("j" . lsp-treemacs-call-hierarchy)
   ("q" . lsp-execute-code-action)
   ("u" . treemacs)
   ;; ("u" . lsp-treemacs-deps-list)
   ("TAB" . lsp-describe-thing-at-point-custom)
   ("SPC" . completion-at-point)
   ("m" . lsp-ui-imenu)
   ("c" . lsp-ui-mode)
   ("r" . lsp-ui-find-next-reference)
   ("g" . lsp-ui-find-prev-reference)
   ))

(xah-fly--define-keys
 (define-prefix-command 'xah-fly-window-keymap)
 '(
...
;;;;
;;aca menjao - dodao sam svoje shortcut-ove
(xah-fly--define-keys
...
;;;;
;;aca dodao komentar - ovo je bilo
;; (xah-fly--define-keys
;;  (define-prefix-command 'xah-fly-leader-key-map)
;;  '(
;;    ("SPC" . xah-fly-insert-mode-activate)
...
;; aca dodao
(defun change-color-and-got-to-line-mode ()
  (interactive)
  (call-interactively #'xah-fly-command-mode-activate)
...
(progn
  ...
  (progn
    ...
    (define-key xah-fly-key-map (kbd "<home>") 'change-color-and-got-to-line-mode)
    ;; (define-key xah-fly-key-map (kbd "<pause>") 'xah-fly-command-mode-activate) ; aca dodao
...
;;aca dodao
(defun change-color-and-insert-char-mode ()
  (interactive)
  (call-interactively #'xah-fly-insert-mode-activate)
...
Version 2017-01-21"
  ...
  (xah-fly--define-keys
   ...
   '(
     ...
     ("w" . xah-backward-left-bracket) ;;aca menjao m u w
     ("n" . forward-char)
     ("o" . open-line)
     ("p" . xah-kill-word)
     ...
     ("t" . next-line)
     ("u" . change-color-and-insert-char-mode)
     ("v" . xah-forward-right-bracket)
     ;; ("w" . xah-next-window-or-frame)
     ("x" . ivy-switch-buffer)
     ("y" . minibuffer-keyboard-quit)
     ;; ("y" . keyboard-escape-quit)
     ;; ("y" . xah-toggle-letter-case)	;;aca menjao "x" u "y"
     ("m" . set-mark-command)		;;aca menjao "y" u "m"
